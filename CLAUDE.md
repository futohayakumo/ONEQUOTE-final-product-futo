...
## Real data, modelled data, and the line between them

The site's claim is that every figure is computed and that every figure can
say where it came from. Keep that true:

- **Fetched, and used as fetched:** ECB reference rates (JPY, EUR, SGD, KRW —
  live at quotation time, `cached` when the ECB does not answer) and
  UN/LOCODE seaports with coordinates (`src/data/ports.ts`, generated by
  `pnpm ports:pull`; never edit by hand). A tariff currency the ECB does not
  quote stays USD and says so. A port the list cannot place is not offered.
- **Modelled, and labelled:** the sea distance (chokepoint legs), the base
  rate and transit derived from it, surcharges, tier discounts, sailings,
  waitlist, service timings, the queueing model. `src/lib/*.ts` says so in
  comments and the pages say so in copy. Do not add a figure without a label.
- **Confirmed by the technical lead (2026-09-16):** the services and their
  grouping, the two gateways, BigQuery isolation, the Bolt cap of 72 working
  hours, and that ×3 / ×5 are an external POC benchmark. `src/lib/flow-data.ts`
  and `src/lib/quality.ts` carry the details; the edges between services are
  still this site's reading and are marked as such.
- **Not measured, and said so:** the team's own throughput/defect multiplier
  and what Bot QA catches. Do not fill those in.

## Architecture notes

- `pnpm boot` brings up PostgreSQL, the API and the site (`--prod` builds);
  `pnpm halt` stops the two servers. `pnpm boot:win` is the Windows twin.
- The API imports the browser's pure libs by relative path
  (`../../../src/lib/*.ts`) so both compute identical figures; the root
  `package.json` is `"type": "module"` for that reason. Those libs may import
  each other but nothing else, and use `.ts` extensions so the node test
  runner executes them directly.
- The API runs through `@swc-node/register` with `SWCRC=true` (via
  `cross-env`). `tsx`/esbuild emit no decorator metadata, and Nest's DI and
  `ValidationPipe` fail silently under them — a request with unknown fields
  was accepted. Use explicit `@Inject(...)`.
- Locale bundles must carry the same keys (`pnpm locales:check`); the i18n
  test forbids blank values, so punctuation goes inside the string, never in
  a separate key.
