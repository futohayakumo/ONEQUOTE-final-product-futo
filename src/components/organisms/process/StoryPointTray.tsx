...
import { StoryPointChip } from "../../molecules/StoryPointChip";
...
export function StoryPointTray({
  ...
  return (
    <div className="flex flex-col gap-4 border border-border bg-studio p-5 rounded-sharp lg:flex-row lg:items-center lg:gap-8">
      <div className="flex flex-col gap-1.5 lg:w-72 lg:shrink-0">
        <span className="type-label text-crimson">{t("sim.tray.title")}</span>
        {/* What the unit IS, before how to use it. Both reviewers pressed a
            chip without knowing what they were throwing. */}
        <span className="type-caption">{t("sim.tray.note")}</span>
