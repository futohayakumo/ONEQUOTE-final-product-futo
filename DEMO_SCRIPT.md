...
- ▶ 「図・コード・ログを開く」→ システムマップ → **Quotation Service** をクリック → 経路が引き直され、右にトレースとログ
...
**「仕事の単位は2週間でした。いまは72時間です。」**

- → 「同じ仕事。違う流れ。」の2枚の絵
- → **「スループットは3倍。欠陥は5倍。」** — ここは正直に：
  **「これは社内報告の実測です。欠陥5倍は悪化ではなく、Bot QA が弾く前提。ただし Bot が実際に何件止めたかは計測されていません。それはこのページにも書いてあります。」**
- ▶ 「シミュレーションを開始」→ AI-DLC に切替 → 「3 SP」を2回クリック → 1.7日で完了
  **「4フェーズの1サイクル。Bolt の両端24時間と72時間は文書の値で、その間の対応だけが私のモデルです」**
...
| 種類 | 例 | 出どころ |
| :-- | :-- | :-- |
| 実測 | スループット×3、欠陥×5 | OTSV 社内報告 |
| 外部の公開データ | 為替、港 | ECB 参照レート、UN/LOCODE |
| 文書の規定値 | Bolt 24〜72時間 | AI-DLC 文書 |
| モデル | 航路の基本運賃、サーチャージ、待ち行列の定数 | 私の仮定。ページにそう書いてある |
...
3. **システム構成** — サイトのシステムマップと C4 は推定。本物とどこが違うか
